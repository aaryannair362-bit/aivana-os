...
today -- and actually report interactions instead of always returning an empty list.
"""
import pytest
...
SCRIBE_EXTRACTION = {
    "chiefComplaint": "Fever", "hpi": "Fever for 3 days", "primaryDiagnosis": "Viral fever",
    "differentialDiagnosis": "", "advice": "Rest and fluids",
    "medications": [{"drugName": "Paracetamol", "dose": "500mg", "frequency": "TID", "route": "Oral", "duration": "5 days"}],
    "labTests": ["CBC"],
}


def test_opd_scribe_auto_creates_tasks_for_linked_patient(client, doctor, admitted_patient, auth_headers, monkeypatch):
    mock_groq_json(monkeypatch, SCRIBE_EXTRACTION)
    resp = client.post("/api/scribe", headers=auth_headers(doctor),
                        json={"transcript": "Doctor: patient has fever for 3 days", "patient_id": admitted_patient})
...
def test_auto_created_tasks_are_assigned_to_the_patients_active_nurse(client, doctor, nurse, admitted_patient, auth_headers, monkeypatch):
    ...
    mock_groq_json(monkeypatch, SCRIBE_EXTRACTION)
    client.post("/api/scribe", headers=auth_headers(doctor),
                json={"transcript": "Doctor: patient has fever for 3 days", "patient_id": admitted_patient})
    ...
    # The assigned nurse must actually be able to complete it.
    complete = client.patch(f"/api/ipd/tasks/{tasks[0]['id']}", headers=auth_headers(nurse), json={"status": "Completed"})
    assert complete.status_code == 200


def test_opd_scribe_does_not_create_tasks_for_unlinked_walkin(client, doctor, auth_headers, monkeypatch):
    """An OPD walk-in with no patient_id (or one that doesn't resolve) has no nurse to hand
    tasks to -- this must be a silent no-op, not an error, and not create phantom tasks."""
    mock_groq_json(monkeypatch, SCRIBE_EXTRACTION)
    resp = client.post("/api/scribe", headers=auth_headers(doctor),
                        json={"transcript": "Doctor: patient has fever for 3 days"})
    assert resp.status_code == 200
    assert resp.json()["tasks_created"] == 0


def test_finalize_regenerates_tasks_without_duplicating(client, doctor, admitted_patient, auth_headers, monkeypatch):
    mock_groq_json(monkeypatch, SCRIBE_EXTRACTION)
    create = client.post("/api/scribe", headers=auth_headers(doctor),
                          json={"transcript": "Doctor: patient has fever for 3 days", "patient_id": admitted_patient})
...
def test_finalize_does_not_touch_manually_created_tasks(client, doctor, head_nurse, admitted_patient, auth_headers, monkeypatch):
...
def test_drug_interactions_endpoint_returns_real_interactions(client, doctor, auth_headers, monkeypatch):
    """Regression for the always-empty bug: _generate_json is dict-only, so a prompt asking
    for a bare JSON array meant isinstance(result, list) was always False and this endpoint
    always reported {"interactions": []} no matter what. Fixed by requesting a
    {"interactions": [...]} object instead."""
    mock_groq_json(monkeypatch, {"interactions": [
        {"drug_pair": "Warfarin - Aspirin", "severity": "Severe", "reason": "Bleeding risk", "recommendation": "Avoid combination"}
    ]})
...
def test_ipd_round_requires_doctor_role(client, head_nurse, admitted_patient, auth_headers):
    resp = client.post("/api/ipd/rounds", headers=auth_headers(head_nurse),
                        json={"patient_id": admitted_patient, "transcript": "Doctor: reviewing patient today"})
    assert resp.status_code == 403


def test_ipd_round_creates_consultation_with_admission_day_and_tasks(client, doctor, admitted_patient, auth_headers, monkeypatch):
    mock_groq_json(monkeypatch, SCRIBE_EXTRACTION)
    resp = client.post("/api/ipd/rounds", headers=auth_headers(doctor),
                        json={"patient_id": admitted_patient, "transcript": "Doctor: reviewing patient on day 1"})
    assert resp.status_code == 200
    data = resp.json()
...
def test_ipd_round_interaction_check_sees_prior_medications_not_just_todays(client, doctor, admitted_patient, auth_headers, monkeypatch):
    """The whole point of a multi-day cycle: today's new drug must be checked against
    everything the patient is already on, not just against itself."""
    mock_groq_json(monkeypatch, {
        "chiefComplaint": "", "hpi": "", "primaryDiagnosis": "", "differentialDiagnosis": "", "advice": "",
        "medications": [{"drugName": "Warfarin", "dose": "5mg", "frequency": "OD", "route": "Oral", "duration": "ongoing"}],
        "labTests": [],
    })
    client.post("/api/ipd/rounds", headers=auth_headers(doctor),
                json={"patient_id": admitted_patient, "transcript": "Doctor: starting Warfarin today"})

    def fake_interaction_call(prompt, system=None, temperature=0.3):
        import json as _json
        if "clinical pharmacologist" in prompt:
            assert "Warfarin" in prompt and "Aspirin" in prompt, "interaction check must include the prior day's medication"
            return _json.dumps({"interactions": [
                {"drug_pair": "Warfarin - Aspirin", "severity": "Severe", "reason": "Bleeding risk", "recommendation": "Avoid"}
            ]})
        # The extraction call for today's transcript ("...adding Aspirin today...").
        return _json.dumps({
            "chiefComplaint": "", "hpi": "", "primaryDiagnosis": "", "differentialDiagnosis": "", "advice": "",
    ...
    monkeypatch.setattr(app_main.scribe, "_call_groq_api", fake_interaction_call)

    resp = client.post("/api/ipd/rounds", headers=auth_headers(doctor), json={
        "patient_id": admitted_patient,
        "transcript": "Doctor: adding Aspirin today for cardioprotection",
    })
    assert resp.status_code == 200
    assert len(resp.json()["interaction_warnings"]) == 1


def test_discontinued_medication_drops_out_of_active_regimen(client, doctor, admitted_patient, auth_headers, monkeypatch):
    mock_groq_json(monkeypatch, {
        "chiefComplaint": "", "hpi": "", "primaryDiagnosis": "", "differentialDiagnosis": "", "advice": "",
        "medications": [{"drugName": "Ibuprofen", "dose": "400mg", "frequency": "BID", "route": "Oral", "duration": "5 days"}],
        "labTests": [],
    })
    round1 = client.post("/api/ipd/rounds", headers=auth_headers(doctor),
                          json={"patient_id": admitted_patient, "transcript": "Doctor: starting Ibuprofen today"})
...
def test_cross_org_patient_id_does_not_get_linked(client, doctor, make_user, auth_headers, monkeypatch):
    ...
    mock_groq_json(monkeypatch, SCRIBE_EXTRACTION)
    resp = client.post("/api/scribe", headers=auth_headers(doctor),
                        json={"transcript": "Doctor: patient has fever for 3 days", "patient_id": other_patient})
    assert resp.status_code == 404


def test_aggregated_tasks_endpoint_scopes_nurse_to_assigned_patients(client, doctor, head_nurse, nurse, make_user, auth_headers, monkeypatch):
    ...
    mock_groq_json(monkeypatch, SCRIBE_EXTRACTION)
    client.post("/api/scribe", headers=auth_headers(doctor), json={"transcript": "Doctor: patient one has fever", "patient_id": p1})
    client.post("/api/scribe", headers=auth_headers(doctor), json={"transcript": "Doctor: patient two has fever", "patient_id": p2})
