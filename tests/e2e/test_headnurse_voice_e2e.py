...
happens after an explicit Stop click uploads and transcribes the clip -- see
tests/e2e/test_ipd_voice_mic_lifecycle.py's module docstring for the same note in more detail.
"""
import json
...
def test_headnurse_mic_button_works_on_unassigned_patient(js_page, live_server_url, unassigned_patient, monkeypatch):
    """The core HeadNurse-vs-Nurse distinction: HeadNurse can voice-consult even on a patient
    nobody is assigned to yet."""
    import app.main as app_main

    head_nurse, patient = unassigned_patient
    queue_transcription_result(monkeypatch, app_main, "patient stable, no complaints")
    ...
    mic_btn.click(force=True)  # stop -> upload -> transcribe
    js_page.wait_for_timeout(300)
...
def test_headnurse_full_voice_round_trip_on_unassigned_patient(js_page, live_server_url, unassigned_patient, monkeypatch):
    ...
    queue_transcription_result(monkeypatch, app_main, "BP 118 over 76, patient comfortable")
...
def test_headnurse_transcription_failure_handled_gracefully(js_page, live_server_url, unassigned_patient, monkeypatch):
    import app.main as app_main
